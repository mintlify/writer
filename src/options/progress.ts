...
export const createProgressTree = async () => {
  ...
  if (!isProgressVisible) {
    const checkProgressVisibility = vscode.window.createTreeView("progress", {
  ...
  try {
    ...
    vscode.window.createTreeView("progress", {
...
export const getActiveIndicatorTypeNames = () => {
  return trackingConfigIds
    .filter((config) => Boolean(vscode.workspace.getConfiguration("docwriter").get(config.id)))
...
class ProgressBar extends vscode.TreeItem {
  constructor(public readonly bar: string, public readonly progress?: Progress) {
    super(bar, vscode.TreeItemCollapsibleState.Collapsed);
    this.id = "progress";
    this.tooltip = "Progress bar (click to toggle settings)";

    this.description =
      progress != null && progress.total !== 0 ? `${progress.current}/${progress.total}` : "None";
  }
}

class ComponentOption extends vscode.TreeItem {
  constructor(config: TrackingConfig, breakdown?: { current: number; total: number }) {
